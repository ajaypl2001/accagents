...
if (isset($_POST["srchClickbtn"])) {
    ...
    header("Location: index.php?getsearch=$search&page_no=1");
}
		...
		<div class="main-div">
			<div class=" admin-dashboard">
				<div class="row ">
					...
					<div class="col-sm-12">
					<form action="vg_payment_export.php" method="post" class="float-right ml-2">
         ...
         $result_count = mysqli_query(
             $con,
             "SELECT COUNT(*) As total_records FROM `st_application` where v_g_r_status='V-G' $agent_id_not_show $searchInput"
         );
         ...
         while ($row_nm = mysqli_fetch_assoc($qurySql)) {
             ...
             $action1 = "<a href='javascript:void(0)' class='btn btn-sm btn-warning add_amt' data-target='#addAmount' data-toggle='modal' data-name='$studentname' data-id='$sno'>Add Amount</a>";
			...
			$start_date = $course_row['commenc_date'];
             ...
             if (mysqli_num_rows($check_res) > 0) {
				...
				if($stud_travelled == '' ){
				 ...
				 $action1 = "<a href='javascript:void(0)' class='btn btn-sm btn-success add_amt' data-target='#addAmount' data-name='$studentname' data-toggle='modal' data-id='$sno'>Add Amount</a>";
             } else {
                 $tot_amt = "";
                 $amt_pending = "";
                 $amt_rec = "";
                 $total_amt_rec = "";
                 $updated_date = "No Action";
                 $last_remarks = "";
                 $follow_sts = "";
             }
             ?>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title">Payment Recieved For <b class="st_name"></b></h4>
			...
			$.post("../response.php?tag=recievedElement", {
				...
				if (d == "") {
					...
					for (i in d) {
						$('<tr>' +
							'<td>' + d[i].sno + '</td>' +
							'<td style="white-space:no-wrap">' + d[i].tot_amt_rec + '-(' + d[i].amt_rec + '+' +d[i].amt_dollar+ '*60 )</td>' +
