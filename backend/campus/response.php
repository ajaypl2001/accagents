...
if($_GET['tag'] == "assignTeacher"){
	...
	for ($i=0;$i<$countValue;$i++) {
		...
		$getQry2 = "UPDATE `` SET `tearcher_assign`='Yes' WHERE `sno` ='$implodeId2[$i]'";
	...
	$getQry3 = "SELECT sno, fname, lname, student_id, email_address FROM `` WHERE sno IN ($getCPL)";
...
if($_GET['tag'] == "getClassSchedule"){
	...
	$rsltQuery = "SELECT sno, fname, lname, student_id, prg_name1, prg_intake, on_off_shore, student_status FROM  where prg_name1!='' AND  on_off_shore!='' AND (v_g_r_status='V-G' OR (on_off_shore='Onshore' AND loa_file!='')) AND (STR_TO_DATE(prg_intake, '%b-%Y')) >= '2024-05-00' AND (tearcher_assign='' OR tearcher_assign='Completed') AND (student_status!='' AND student_status!='Not started') $getPName3";
