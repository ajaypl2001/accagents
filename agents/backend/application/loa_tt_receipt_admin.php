...
$loa_receipt_id_admin = $rowstr1['loa_receipt_id_admin'];
...
if(empty($loa_receipt_id_admin)){
    ...
    $loa_receipt_id_2 = $loa_receipt_id_admin;    
}
mysqli_query($con, "update `st_application` set loa_receipt_id_admin='$loa_receipt_id_2' where `sno`='$snoid'");
...
Tuition Fee Received: $'.$genrate_amount_loa_admin.'.00 CAD<br><Br><Br><Br>
...
if(!empty($loa_receipt_file_admin)){
	unlink("../LOA_Receipt/$loa_receipt_file_admin");
}

$firstname = str_replace(' ', '', $fname);
$loa_rcpt_name = 'LOA_Receipt_NEW_'.$firstname.'_'.$refid;
$loa_rcpt_name2 = 'LOA_Receipt_NEW_'.$firstname.'_'.$refid.'.pdf';
$filepath = "../LOA_Receipt/$loa_rcpt_name.pdf";

file_put_contents($filepath, $document->output());

mysqli_query($con, "update `st_application` set `loa_receipt_file_admin`='$loa_rcpt_name2', `genrate_amount_loa_admin`='$genrate_amount_loa_admin', `genrate_amount_loa_date_admin`='$col_updated' where `sno`='$snoid'");
