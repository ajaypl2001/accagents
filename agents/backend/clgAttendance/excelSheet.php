...
if(!empty($_POST['warningInput'])){
	$warningInput2 = $_POST['warningInput'];
	$warningInput4 = "AND start_college_attendance.warning_letter='$warningInput2'";
}else{
	$warningInput2 = '';
	$warningInput4 = '';
}
...
if($role2 == 'Actual_Details'){
	$slctQry_2 = "SELECT * FROM st_application where loa_file!='' AND loa_file_status='1' $keywordDiv";
	...
	$slctQry_2 = "SELECT * FROM st_application where v_g_r_status='V-G' $keywordDiv $getIntake3";
	
	$adHeading = "\t In/W/D \t Inprogress Status \t Refund Type \t Refund Processed \t Refund Amount \t Followup Status \t Followup Date \t Remarks \t Updated On \t Added By";
	
}elseif($role2 == 'Attendance_Reports'){
	$slctQry_2 = "SELECT st_application.sno, st_application.fname, st_application.lname, st_application.refid, st_application.student_id, st_application.prg_name1, st_application.prg_intake, st_application.email_address, st_application.on_off_shore, st_application.user_id, st_application.dob, st_application.passport_no, st_application.mobile, st_application.address1, st_application.prg_name1, st_application.country, st_application.passport_no, start_college_attendance.app_id, start_college_attendance.warning_letter
FROM st_application
INNER JOIN start_college_attendance ON st_application.sno=start_college_attendance.app_id
WHERE start_college_attendance.app_id!='' $keywordDiv2 $warningInput4
ORDER BY start_college_attendance.sno ASC";	
	$adHeading = '';
	...
	$slctQry_2 = "SELECT * FROM st_application where (v_g_r_status='V-G' OR (on_off_shore='Onshore' AND loa_file!='')) $keywordDiv  $getIntake4";
	$adHeading = '';
}
...
echo 'S.No.' ."\t". 'Agent Name' ."\t". 'Fullname' . "\t" .'Student Id'. "\t" .'Ref Id'. "\t" .'DOB' . "\t" .'Std Location' . "\t" .'Mobile Number'. "\t" .'Email Address'. "\t" .'Passport No.'. "\t" .'Country'. "\t" .'On/Off'. "\t" .'Program Name'. "\t" .'Program Start Date'. "\t" .'Program End Date'. "\t" .'warning_letter_1st'. "\t" .'warning_letter_2nd'. "\t" .'Confirmation_of_Dismissal'. "\t" .'Confirmation_of_Enrollment'. "\t" .'Confirmation_of_Withdrawal'. "\t" .'Letter_of_Completion'. "\t" .'Student_Dismissal_Letter'.$adHeading."\n";
...
while ($row = mysqli_fetch_assoc($checkQuery_2)) {
		...
		$warning_letter_2nd = '';
		$warning_letter_1st = '';
		$Confirmation_of_Dismissal = '';
		$Confirmation_of_Enrollment = '';
		$Confirmation_of_Withdrawal = '';
		$Letter_of_Completion = '';
		$Student_Dismissal_Letter = '';
		if(mysqli_num_rows($queryRslt4)){
			while($row_nm4 = mysqli_fetch_assoc($queryRslt4)){
				$getWL = $row_nm4['warning_letter'];
				if($getWL == '2nd_warning_letter'){
					$warning_letter_2nd .= 'Yes';
				}
				if($getWL == '1st_warning_letter'){
					$warning_letter_1st .= 'Yes';
				}
				if($getWL == 'Confirmation_of_Dismissal'){
					$Confirmation_of_Dismissal .= 'Yes';
				}
				if($getWL == 'Confirmation_of_Enrollment'){
					$Confirmation_of_Enrollment .= 'Yes';
				}
				if($getWL == 'Confirmation_of_Withdrawal'){
					$Confirmation_of_Withdrawal .= 'Yes';
				}
				if($getWL == 'Letter_of_Completion'){
					$Letter_of_Completion .= 'Yes';
				}
				if($getWL == 'Student_Dismissal_Letter'){
					$Student_Dismissal_Letter .= 'Yes';
				}
			}
		}else{
			$warning_letter_2nd .= '';					
	...
	$clg_serial_no_bkup = $row['clg_serial_no'];
echo $clg_serial_no_bkup ."\t" . $username ."\t" . $fullname. "\t" . $student_id. "\t" . $refid. "\t" . $dob. "\t" . $on_off_shore. "\t" . $mobile. "\t" . $email_address. "\t" . $passport_no. "\t" . $country. "\t" . $countryDiv. "\t" . $prg_name1. "\t" . $start_date. "\t" . $end_date. "\t" . $warning_letter_1st. "\t" . $warning_letter_2nd. "\t" . $Confirmation_of_Dismissal. "\t" . $Confirmation_of_Enrollment. "\t" . $Confirmation_of_Withdrawal. "\t" . $Letter_of_Completion. "\t" . $Student_Dismissal_Letter .$crsCompletion. "\n";
}
