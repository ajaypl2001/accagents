...
if(isset($_POST['submitbtn'])){
	...
	$snoid = $_POST['snoid'];
	$contract_datetime22 = $_POST['contract_datetime'];
	$remarks = mysqli_real_escape_string($con, $_POST['contract_remarks']);
	$datetime_at = date('Y-m-d H:i:s');
	$date_at = date('Y-m-d');

	$getInQry2 = "UPDATE `st_app_more` SET `contract_status`='$status', `contract_datetime`='$contract_datetime22', `contract_remarks`='$remarks' WHERE `app_id`='$snoid'";
	mysqli_query($con, $getInQry2);
	
	$getInQry4 = "UPDATE `both_main_table` SET `contract_docs_status`='$status' WHERE `sno`='$snoid'";
	mysqli_query($con, $getInQry4);
	
	if($status == 'Accept'){
		header("Location: ../../../docsign/HCOA-EC.php?tab=DirectorSign&stsno=$snoid");
	}else{	
...
$resultQuery4 = "SELECT sno, fname, lname, student_id, refid, email_address, dob, passport_no, idproof, prg_name1, prg_intake, vg_copy_file, fh_re_lodgement FROM both_main_table where sno='$stid'";
...
if(mysqli_num_rows($getQryRslt)){
	...
	$GenerateBtn = 'success';
	$GenerateDownload = '<a href="../../docsign/'.$stud_enrld.'" download>Download Enrollment Contract</a>';	
	
	if(!empty($send_date)){
		$student_signature = '<a href="localhost/accagents/docsign/Student_Sign/'.$student_signature2.'" download>Download<a/>';	
	}else{
	...
	if(!empty($signed_contract2)){
		...
		$signed_contract = '<a href="localhost/accagents/docsign/uploadsWS/'.$signed_contract2.'" download>Download<a/>';
	}else{
	...
	$contract_datetime33 = $rowS['contract_datetime'];
	...
	if(!empty($contract_datetime33)){
		$contract_datetime3 = $contract_datetime33;
	}else{
	...
	$statusDocs = '1) Send Email to Student- <span class="text-danger">Pending</span><br>';
	$statusDocs2 = '2) Signed By Student- <span class="text-danger">Pending</span>';
...
<div class="row">
<?php if($sessionid1 != '3716' && $sessionid1 != '3738' && $sessionid1 != '3716'){ ?>
<div class="col-12 col-lg-4 col-xl-3 col-md-6 mb-2">
<a href="vgDocsAgent.php?stid=<?php echo $stid; ?>"><i class="fas fa-arrow-left"></i> Back to Travel Support Lists(VG)</a>
</div>
<?php } ?>
...
<div class=" application-tabs row pt-4">
	...
	<tr>	
		...
		<td>
		<form action="localhost/accagents/docsign/HCOA-EC.php" method="post" autocomplete="off">
	...
	<tr>	
		...
		<form method="post" action="">			
			<input type="text" name="contract_datetime" class="form-control datepicker123 mb-2" value="<?php echo $contract_datetime33; ?>" placeholder="Select Director Date" required>
			<select name="contract_status" class="form-control form-control-sm mb-2" required>
				<option value="">Select Status</option>
				<option value="Accept">Accept</option>
				<option value="Reject">Reject</option>
			</select>
			<textarea name="contract_remarks" class="form-control form-control-sm mb-2" cols="10"></textarea>
			<input type="hidden" name="snoid" value="<?php echo $stid; ?>" required>
	...
	<tr>	
		...
		<td><?php echo $contract_datetime33; ?></td>
	</tr>
	<tr>	
...
$(document).on('click', '.sendbtnClick', function(){
	...
	$.post("sendStudentSign.php",{"stusno":stusno, "getEmail":getEmail},function(d){
		if(d=='1'){
			...
			window.location.href = "localhost/accagents/agents/backend/clg_docs/signedContractSend.php?stid="+stusno+"";
			return true;
		}
		if(d=='4'){
