...
// if(!empty($_GET['btchId'])){
// 	$btchId = $_GET['btchId'];
// 	$rsltQuery33 = "SELECT * FROM `m_batch` WHERE sno='$btchId'";
// 	$getQrry = mysqli_query($con, $rsltQuery33);
// 	$rowCnt = mysqli_num_rows($getQrry);
// }else{
// 	header("Location: ../../login");
//     exit();
// }

// if(($roles1 == 'ClgCM') && !empty($btchId) && $rowCnt){
...
WHERE m_student.teacher_id!='' AND m_student.betch_no='$btchId' $searchInput");
...
FROM m_student
INNER JOIN both_main_table ON m_student.app_id=both_main_table.sno
WHERE m_student.teacher_id!='' AND m_student.betch_no='$btchId' $searchInput order by m_student.updated_on DESC LIMIT $offset, $total_records_per_page";
?>
<style>
	.error_color {
...
<section class="container-fluid">
	<div class="main-div card">
		<div class="card-header">
			<h3 class="my-0 py-0">Batch Assign Student Lists</h3>
		</div>
		<div class="card-body">
			<div class="row justify-content-between">
				...
				<!--form method="POST" action="excelSssheetxxx.php" autocomplete="off">
							...
							<tbody>
								...
								if (mysqli_num_rows($qurySql)) {
									while ($row_nm = mysqli_fetch_assoc($qurySql)) {
										...
										$module_name = $rowSC33['module_name'];
										$module_code = $rowSC33['module_code'];
										...
										<tr>
											...
											<td style="white-space: nowrap;"><?php echo $module_code . ' ' . $module_name; ?></td>
